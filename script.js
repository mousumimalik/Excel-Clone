...
// when browser is completely loaded then run the code whithin this
$(document).ready(function () {
    ...
    for(let i = 1; i <= 100; i++) {
        ...
        for(let j = 1; j <= 100; j++) {
            ...
            let colCode = $(`.colId-${j}`).attr("id").split("-")[1]; /* get the column code at 1st index */
            
            let column = $(`<div class="input-cell" contenteditable="false" id="row-${i}-col-${j}" data="code-${colCode}"></div>`); /* contenteditable="false" -can't select the cells */
    ...
    $(".style-icon").click(function(){
        $(this).toggleClass("selected"); /* toggleClass -if not selected then select & if selected then select */
    });
    ...
    $(".input-cell").click(function(e){
        ...
        if(e.ctrlKey) {
            ...
            if(rowId > 1){
                let topCellSelected = $(`#row-${rowId - 1}-col-${colId}`).hasClass("selected"); /* hasClass return true/false */
                if(topCellSelected){
            ...
            if(rowId < 100){
                let bottomCellSelected = $(`#row-${rowId + 1}-col-${colId}`).hasClass("selected"); /* hasClass return true/false */
            ...
            if(colId > 1){
                let leftCellSelected = $(`#row-${rowId}-col-${colId - 1}`).hasClass("selected"); /* hasClass return true/false */
                if(leftCellSelected){
            ...
            if(colId < 100){
                let rightCellSelected = $(`#row-${rowId}-col-${colId + 1}`).hasClass("selected"); /* hasClass return true/false */
                if(rightCellSelected){
                    $(this).addClass("right-cell-selected");
                    $(`#row-${rowId}-col-${colId + 1}`).addClass("left-cell-selected");
                }
            }
            //$(this).addClass("selected"); -repeatedd
        }
        else {
        ...
        $(this).addClass("selected"); // not repeated
        changeHeader(this);
    });

    function changeHeader(ele){
        let [rowId, colId] = getRowCol(ele);
        let cellInfo = defalutProperties;
    ...
    $(".input-cell-container").scroll(function(){
        ...
        $(".column-name-container").scrollLeft(this.scrollLeft); /* column-name-container scrolling with input-cell / scrollLeft -x axis scroll */
        $(".row-name-container").scrollTop(this.scrollTop) /* there are ony 2 type of scroll i.e scrollLeft & scrollTop */
    })
...
function updateCell(property, value, defalutPossible) {
    $(".input-cell.selected").each(function() {
        ...
        if(cellData[selectedSheet][rowId]){
            if(cellData[selectedSheet][rowId][colId]){
                ...
                cellData[selectedSheet][rowId][colId] = {...defalutProperties};
                cellData[selectedSheet][rowId][colId][property] = value;
            }
        }else{
        ...
        //console.log(defalutPossible.JSON.stringify(cellData[selectedSheet][rowId][colId]) == JSON.stringify(defalutProperties));

        if(defalutPossible && (JSON.stringify(cellData[selectedSheet][rowId][colId]) == JSON.stringify(defalutProperties))){
            //console.log("hello");
...
$(".icon-bold").click(function() {
    if($(this).hasClass("selected")) {
        updateCell("font-weight", "", true); /* to remove bold property just add empty string "" */
    }
    else{
...
$(".icon-italic").click(function() {
    if($(this).hasClass("selected")) {
        updateCell("font-style", "", true); /* to remove bold property just add empty string "" */
    }
    else{
...
$(".icon-underline").click(function() {
    if($(this).hasClass("selected")) {
        updateCell("text-decoration", "", true); /* to remove bold property just add empty string "" */
    }
    else{
...
function addSheetEvents() {
    ...
    $(".sheet-tab.selected").contextmenu(function(e) {
        e.preventDefault(); // stop opening bydefault model | stop bydefault work | nothing happens when right click on sheet1
       // console.log("HELLO");
       // console.log(e);
       // console.log($(".sheet-options-model").length);
        ...
        $(".sheet-delete").click(function() {
            if(Object.keys(cellData).length > 1){
               ...
               // $(".sheet-tab.selected").remove();
                ...
                alert("Sorry, there is only sheet. so it is not possible.")
            }
        });
    }
...
$(".icon-paste").click(function(){
  ...
  // console.log( $(".input-cell.selected")[0]);
     ...
     for(let cell of selectedCells){
        ...
        //         cellData[selectedSheet][newColId - 1][newColId - 1] = {...cellData[selectedSheet][rowId - 1][colId - 1]};
        //  }
        //  else{
        //         cellData[selectedSheet][newRowId - 1] = {};
        //         cellData[selectedSheet][newColId - 1][newColId - 1] = {...cellData[selectedSheet][rowId - 1][colId - 1]};
        //  }
